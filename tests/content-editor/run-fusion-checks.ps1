...
try {
  ...
  $result = Join-Path $PSScriptRoot 'fusion-smoke/result.txt'
  Set-Content -LiteralPath $result -Value 'RUNNING'
  $process = Start-Process -FilePath (Join-Path $workspace 'love/love.exe') -ArgumentList 'tests/content-editor/fusion-smoke' -WindowStyle Hidden -PassThru
  if (-not $process.WaitForExit(50000)) { $process.Kill(); throw 'Fusion tests timed out' }
  $output = Get-Content -LiteralPath $result -Raw
  Write-Output "$Game`: $output"
  if (-not $output.StartsWith('PASS:')) { throw 'Fusion tests failed' }
