...
try {
  ...
  $result = Join-Path $PSScriptRoot 'advanced-forms-smoke/result.txt'
  Set-Content -LiteralPath $result -Value 'RUNNING'
  $process = Start-Process -FilePath (Join-Path $workspace 'love/love.exe') -ArgumentList 'tests/content-editor/advanced-forms-smoke' -WindowStyle Hidden -PassThru
  if (-not $process.WaitForExit(50000)) { $process.Kill(); throw 'Advanced forms tests timed out' }
  $output = Get-Content -LiteralPath $result -Raw
  Write-Output "$Game`: $output"
  if (-not $output.StartsWith('PASS:')) { throw 'Advanced forms tests failed' }
