...
try {
  ...
  $result = Join-Path $PSScriptRoot 'bridges-smoke/result.txt'
  Set-Content -LiteralPath $result -Value 'RUNNING'
  $process = Start-Process -FilePath (Join-Path $workspace 'love/love.exe') -ArgumentList 'tests/content-editor/bridges-smoke' -WindowStyle Hidden -PassThru
  if (-not $process.WaitForExit(50000)) { $process.Kill(); throw 'Bridge tests timed out' }
  $output = Get-Content -LiteralPath $result -Raw
  Write-Output "$Game`: $output"
  if (-not $output.StartsWith('PASS:')) { throw 'Bridge tests failed' }
