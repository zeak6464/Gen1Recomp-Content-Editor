...
local function report(s) local f=assert(io.open(root.."/tests/content-editor/dynamic-forms-smoke/result.txt","w"));f:write(s);f:close() end
...
function love.load()
  ...
  dofile(root.."/tests/content-editor/test_gen3_dynamic_forms.lua")(data,root,lib.PHYSFS_mount,game)
  report("PASS: held-item forms, gender forms, stat/HP preservation, give/take item, custom types, exported reload and validation")
  love.event.quit()
end
