...
local function report(s) local f=assert(io.open(root.."/tests/content-editor/connections-smoke/result.txt","w"));f:write(s);f:close() end
...
function love.load()
  ...
  dofile(root.."/tests/content-editor/test_gen3_connections.lua")(data,root,lib.PHYSFS_mount,game)
  report("PASS: multiple connections, Water Path recovery, reciprocal edits, export and native crossings")
  love.event.quit()
end
