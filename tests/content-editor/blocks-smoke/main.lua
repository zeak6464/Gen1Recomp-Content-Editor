...
local function report(s) local f=assert(io.open(root.."/tests/content-editor/blocks-smoke/result.txt","w"));f:write(s);f:close() end
...
function love.load()
  ...
  dofile(root.."/tests/content-editor/test_gen3_blocks_smoke.lua")(data,root,game)
  report("PASS: new and edited blocks, your own tiles, the map editor picker and the in-game atlas")
  love.event.quit()
end
