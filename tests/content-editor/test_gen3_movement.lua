return function(data,root)
  ...
  C.field=function(_,opts) opts.onPick("Walk left") end
  ...
  C.field=function(_,opts) if opts.title=="ADD COMMAND" then opts.onPick("applymovement") end end
  ...
  assert(commands[1].op=="applymovement" and commands[2].op=="end","New commands must run before script termination")
  local f=assert(io.open(root.."/tests/content-editor/gen3-smoke/movement-editor.png","wb"))
