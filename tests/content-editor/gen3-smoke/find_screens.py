import os, pathlib, struct, json, hashlib, re
...
assets={};targets={}
rules=(root.parent/'graphics_file_rules.mk').read_text()
counts=dict((k,int(v)) for k,v in re.findall(r'\$\(MAPPREVIEWGFXDIR\)/([^/]+)/tiles\.4bpp:[^\n]*\n[^\n]*-num_tiles (\d+)',rules))
for group,folder in [('credits','credits')]+[('areas/'+p.name,'map_preview/'+p.name) for p in (root/'map_preview').iterdir() if p.is_dir()]:
 for p in (root/folder).glob('*.png'):
  ...
  if group.startswith("areas/") and group.split("/")[1] in counts:raw=raw[:counts[group.split("/")[1]]*32]
...
pathlib.Path('tests/content-editor/gen3-smoke/screen-offsets.json').write_text(json.dumps(result,indent=2))
