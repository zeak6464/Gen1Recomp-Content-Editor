...
local function report(s) local f=assert(io.open(root.."/tests/content-editor/form-rules-smoke/result.txt","w"));f:write(s);f:close() end
...
function love.load()
  ...
  dofile(root.."/tests/content-editor/test_gen3_form_rules.lua")(data,root,lib.PHYSFS_mount,game)
  report("PASS: form rules, real battle moves, reversion, field items, known moves, validation")
  love.event.quit()
end
