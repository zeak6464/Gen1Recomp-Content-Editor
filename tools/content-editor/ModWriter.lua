...
local function fieldName(prefix, key)
  ...
  return prefix .. "[" .. escapeStr(tostring(key)) .. "]"
end

local function appendReturnFn(lines, assign, value, pad)
  pad = pad or "  "
  local chunk = {}
  writeValue(value, 0, chunk)
  lines[#lines + 1] = pad .. assign .. " = (function() return "
...
function ModWriter.serializeProject(project)
  ...
  local keys = {}
  for k in pairs(scrubbed) do keys[#keys + 1] = k end
  table.sort(keys, function(a, b) return tostring(a) < tostring(b) end)
  -- LuaJIT allows 65536 constants per function. One giant return { ... }
  -- overflows on large Map Builder projects; each bag/entry is its own chunk.
  local lines = {
  ...
  for _, k in ipairs(keys) do
    if type(k) == "string" or type(k) == "number" then
      local v = scrubbed[k]
      local assign = fieldName("p", k)
      if PROJECT_SPLIT_BAGS[k] and type(v) == "table" then
        lines[#lines + 1] = "  " .. assign .. " = (function()"
        lines[#lines + 1] = "    local t = {}"
        local sub = {}
        for sk in pairs(v) do sub[#sub + 1] = sk end
        table.sort(sub, function(a, b) return tostring(a) < tostring(b) end)
        for _, sk in ipairs(sub) do
          appendReturnFn(lines, fieldName("t", sk), v[sk], "    ")
        end
        lines[#lines + 1] = "    return t"
        lines[#lines + 1] = "  end)()"
      else
        appendReturnFn(lines, assign, v)
      end
    end
  end
...
-- Rewrite a one-table editor_project.lua that LuaJIT cannot compile
-- (more than 65536 constants in the main chunk).
function ModWriter.rewriteOversizeProject(src)
  if type(src) ~= "string" or src == "" then
    return nil, "empty project file"
  end
  if src:find("return %(function%(%)", 1, false) then
    return src
  end
  local function skipSpace(s, i)
    ...
    while i <= n do
      ...
      if c == 32 or c == 9 or c == 13 or c == 10 then
        i = i + 1
      elseif c == 45 and s:byte(i + 1) == 45 then
        while i <= n and s:byte(i) ~= 10 do i = i + 1 end
      else
        break
      end
    end
  ...
  local function skipValue(s, i)
    ...
    if c == 123 then
      ...
      while i <= n and depth > 0 do
        ...
        if b == 34 then
          i = skipString(s, i)
        elseif b == 45 and s:byte(i + 1) == 45 then
          while i <= n and s:byte(i) ~= 10 do i = i + 1 end
        elseif b == 123 then
          depth = depth + 1
          i = i + 1
    ...
    if c == 34 then return skipString(s, i) end
    local n = #s
    while i <= n do
      local b = s:byte(i)
      if b == 44 or b == 125 or b == 10 or b == 13 or b == 32 or b == 9 then
        break
      end
      i = i + 1
    end
  ...
  local function parseEntries(s, openIdx)
    local i = skipSpace(s, openIdx + 1)
    local entries = {}
    local n = #s
    while i <= n do
      i = skipSpace(s, i)
      if i > n or s:byte(i) == 125 then break end
      local key
      if s:byte(i) == 91 then
        local ks = i + 1
        i = skipValue(s, ks)
        key = s:sub(ks, i - 1):match("^%s*(.-)%s*$")
        i = skipSpace(s, i)
        if s:byte(i) == 93 then i = i + 1 end
      ...
      i = skipSpace(s, i)
      if s:byte(i) ~= 61 then
        return nil, "expected '=' after " .. tostring(key)
      end
      i = skipSpace(s, i + 1)
      local v0 = i
      i = skipValue(s, i)
      if not key or key == "" then
        return nil, "missing table key"
      end
      entries[#entries + 1] = { key = key, from = v0, to = i - 1 }
      i = skipSpace(s, i)
      if s:byte(i) == 44 then i = i + 1 end
    end
    return entries
  end
  local ret = src:find("return", 1, true)
  if not ret then return nil, "no return table" end
  local open = src:find("{", ret, true)
  if not open then return nil, "no return table" end
  local top, topErr = parseEntries(src, open)
  if not top then return nil, topErr end
  local out = {
    "-- Auto-generated by the Gen1Recomp content editor.  Edit in the editor",
    "-- and save again to regenerate this file. Manual edits are overwritten.",
    "return (function()",
  ...
  for _, e in ipairs(top) do
    local body = src:sub(e.from, e.to)
    local assign = fieldName("p", e.key)
    if PROJECT_SPLIT_BAGS[e.key] and body:sub(1, 1) == "{" then
      local kids, kidErr = parseEntries(body, 1)
      if not kids then return nil, kidErr end
      out[#out + 1] = "  " .. assign .. " = (function()"
      out[#out + 1] = "    local t = {}"
      for _, kid in ipairs(kids) do
        out[#out + 1] = "    " .. fieldName("t", kid.key)
          .. " = (function() return " .. body:sub(kid.from, kid.to)
          .. " end)()"
      end
      out[#out + 1] = "    return t"
      out[#out + 1] = "  end)()"
    else
      out[#out + 1] = "  " .. assign .. " = (function() return "
        .. body .. " end)()"
    end
  end
  out[#out + 1] = "  return p"
...
function ModWriter.emitMain(project, baseData, derivedModId)
  ...
  if gen2 then
    ...
    if type(project.oakSpeech) == "table" and next(project.oakSpeech) then
      ...
      out[#out + 1] = "      end"
      out[#out + 1] = "    end)"
      -- OakSpeech.new loads data.marillPic (Crystal ships wooper.png) and
      -- ignores a demoSpecies-only patch. Resolve the pic after construction.
      out[#out + 1] = "    local OS = require(\"src.ui.gen2.OakSpeech\")"
      out[#out + 1] = "    local orig = OS.new"
      out[#out + 1] = "    function OS.new(game, opts)"
      ...
      out[#out + 1] = "      end"
      out[#out + 1] = "      local function loadMon(species)"
      out[#out + 1] = "        if type(species) ~= \"string\" or species == \"\" then return nil end"
      out[#out + 1] = "        local data = game and game.data"
      out[#out + 1] = "        local def = data and data.pokemon and data.pokemon[species]"
      out[#out + 1] = "        local img = load(def and def.spriteFront)"
      out[#out + 1] = "        if img then return img end"
      out[#out + 1] = "        local ok, Sprites = pcall(require, \"src.pokemon.Sprites\")"
      out[#out + 1] = "        if not (ok and Sprites and Sprites.path) then return nil end"
      out[#out + 1] = "        local path"
      out[#out + 1] = "        ok, path = pcall(Sprites.path, data, species, \"front\","
      out[#out + 1] = "          { kind = \"oak\" })"
      out[#out + 1] = "        return ok and load(path) or nil"
      out[#out + 1] = "      end"
      out[#out + 1] = "      local oak, player = load(_oak.oakPic), load(_oak.playerPic)"
      out[#out + 1] = "      local playerF = load(_oak.playerPicFemale)"
      out[#out + 1] = "      local sh1, sh2 = load(_oak.shrink1), load(_oak.shrink2)"
      out[#out + 1] = "      if oak then self.oakPic = oak end"
      out[#out + 1] = "      if player then self.playerPic = player end"
      out[#out + 1] = "      if playerF then self.playerPicFemale = playerF end"
      out[#out + 1] = "      if sh1 then self.shrinkPic1 = sh1 end"
      out[#out + 1] = "      if sh2 then self.shrinkPic2 = sh2 end"
      out[#out + 1] = "      if type(_oak.music) == \"string\" and _oak.music ~= \"\" then"
      out[#out + 1] = "        self.music = _oak.music"
      out[#out + 1] = "      end"
      out[#out + 1] = "      local species = _oak.demoSpecies"
      out[#out + 1] = "      if type(species) == \"string\" and species ~= \"\" then"
      out[#out + 1] = "        self.demoSpecies = species"
      out[#out + 1] = "        local ok, Palettes = pcall(require, \"src.world.gen2.Palettes\")"
      out[#out + 1] = "        if ok and Palettes and Palettes.monColors then"
      out[#out + 1] = "          self.marillColors = Palettes.monColors(self.palettes, species)"
      out[#out + 1] = "        end"
      out[#out + 1] = "      end"
      out[#out + 1] = "      local demo = load(_oak.marillPic) or load(_oak.demoPic)"
      out[#out + 1] = "      if not demo then demo = loadMon(self.demoSpecies) end"
      out[#out + 1] = "      if demo then self.marillPic = demo end"
